...
textToCheckIfOnlyVowels - used to keep user input, so that the program could use it in control flow. Returns strings.
...
onlyVowelsOrNotResult - flag variable, shown in the final alert as true or false.
True - if the user text only contains vowels. False - if user text contains not only vowels.

*/
let textToCheckIfOnlyVowels = null;
while (textToCheckIfOnlyVowels === null || textToCheckIfOnlyVowels === "") {
  textToCheckIfOnlyVowels = prompt(
...
let onlyVowelsOrNotResult = true;
for (let character of textToCheckIfOnlyVowels) {
  let vowel = vowels.includes(character.toLowerCase());
  if (!vowel) {
    onlyVowelsOrNotResult = false;
    break;
  }
}

alert(onlyVowelsOrNotResult);
